...
| `bin/gen-root.sh` | Crée ou valide l'autorité racine, son arborescence, sa clé privée, son certificat et ses métadonnées. | `bin/pki-env.sh`, `openssl`, `awk`, `sed`, `mktemp`, `date` |
...
| `bin/revoke-intm-and-leafs.sh` | Révoque une AC intermédiaire puis révoque en lot les certificats finaux sélectionnés dans son historique, avec mise à jour optionnelle des CRL racine et intermédiaire. | `bin/pki-env.sh`, `openssl`, `awk`, `grep`, `mktemp`, `install`, `rm` |
...
| `bin/recovery.sh` | Affiche les opérations en attente et enregistre un examen manuel terminé. | Bash, outils de fichiers |
| `bin/crl.sh` | Opérations CRL racine, courantes ou historiques sous verrou. | OpenSSL, fonctions partagées |
